    ...
    def __init__(self):
        try:
            ...
            try:
                ...
                self.dm = Dispatch('dm.dmsoft')  # 调用大漠插件
                print('自动注册dm.dll - os.system方案')
            except:
                try:
                    subprocess.call(cmd='start regist_dm_admin.bat', shell=True)
                    self.dm = Dispatch('dm.dmsoft')  # 调用大漠插件
                    print('自动注册dm.dll - 最终方案')
                except:
                    tt.MessageBox('自动注册大漠插件失败,请手动注册!')
                    # print('注册大漠插件失败,请手动注册!')
